        ...
        PHPhotoLibrary.shared().performChanges {
            ...
            if !success {
                ...
                DispatchQueue.main.async {
                    let alert = UIAlertController(title: "Imagem salva", message: "Sua image foi salva no álbum de fotos", preferredStyle: .alert)
    ...
    @IBAction func save(_ sender: UIButton) {
        PHPhotoLibrary.requestAuthorization { (status) in
            ...
            case .authorized:
                self.saveToAlbum()
            default:
                let alert = UIAlertController(title: "ERRO", message: "Você precisa autorizar o aceso ao álbum para poder salvar sua foto.", preferredStyle: .alert)
                let okAction = UIAlertAction(title: "OK", style: .cancel, handler: nil)
                alert.addAction(okAction)
                self.present(alert, animated: true, completion: nil)
            }
